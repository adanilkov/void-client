...
struct CommonData
{
	...
	inline static std::vector<std::string> friendsList;
	...
	static bool isFriend(CommonData::PlayerData player)
	{
		for (std::string fr : friendsList)
		{
			if (fr == player.name) return true;
		}
		return false;
	}
